...
<!-- Continue from here after fixing  ea_homogeneous_area -->
```{r, warning=F}
infra_agg_discrete_vect <- eaTools::ea_homogeneous_area(infra_agg_discrete, groups = infrastructureIndex)
```
...
As can be seen in the figure above, `st_warp` merges grid cells that have the same value, and the vectorized raster doesn't not end up being that big in the end.

## Chech national distribution
...
```{r}
infra_agg_discrete_vect <- st_transform(infra_agg_discrete_vect, 25833)
```
...
infra_stats <- readRDS("P:/41201785_okologisk_tilstand_2022_2023/data/cache/infra_stats.rds")
```
