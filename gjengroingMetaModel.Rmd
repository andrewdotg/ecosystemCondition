...
The workflow spans two platforms including RStudio and Python.
...
output_dir <- "/data/P-Prosjekter2/412421_okologisk_tilstand_2024/Andrew/meta/HighResCanopyHeight-main/DataDownload/testHarstad/tiles"
...
Here we include a shapefile of our area of intertest. The script will loop through all features in the shapefile, calculate their extent and query the WMS to download orthophotos within that extent in 256 x 256 pixel tiles. 
```{r}
ROIs <- vect("/data/P-Prosjekter2/412421_okologisk_tilstand_2024/Andrew/meta/HighResCanopyHeight-main/DataDownload/testHarstad/aoi.shp")
RegionPolygonUTM33N <- st_as_sf(ROIs)
if (st_crs(RegionPolygonUTM33N)$epsg != 25833) {
  RegionPolygonUTM33N <- st_transform(RegionPolygonUTM33N, crs = 25833)
}
...
mclapply(1:nrow(RegionPolygonUTM33N), function(i) {
  geometry <- RegionPolygonUTM33N[i, ]
  id_field <- RegionPolygonUTM33N$id[i]  # Adjust field name if necessary
