...
```{r kommuner, fig.cap="Municipalities in Norway."}
...
```{r alien-normalise, fig.cap="Performing a linear break-point type normalisation of the slitasje variable.", fig.width=4}
...
I will subset the `alien_data` into the three ecosystems. Note that
...
If we say that we need 20 data points for each HIA and municipality combintaion in order to extrapolate to the entire HIA for that municipality, we get much less to work with:
...
There is a trend here, but not very strong.
...
and justifies using the HIA+municipality intersections as local reference
areas.
