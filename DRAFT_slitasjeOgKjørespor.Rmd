...
I want to look at if the indicator values can be averages over smaller
and more meaningful areas (ecological homogeneous areas) than the 5
default regions.
...
zones by region, and then merge sones that are very uncommon in a
spesific region.
...
to assume that the polygons are representative outside of the mapped
area.
...
-   each vegetation zone (e.g. boreonemal zone in Østlandet; excluding
    zone with too little data points)
-   each vegetation region (aka bioregion, e.g. boreonemal zone and
    continental section in Østlandet)
-   each pixel of a give size, say 1 x 1 km

The last option is problematic because we often will have too few points
to be able to estimate credible mean. I can try the second option first,
and see how many data points we get for each zone.
