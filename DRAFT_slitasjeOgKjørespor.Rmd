...
It's easuer to see what's happeing if we zoom in a bit. 
...
I want to look at wheter the indicator values can be averages over
smaller and more meaningful areas (ecological homogeneous areas) than
the 5 default regions. One option is this data set with vegetation zone.
...
It could make sense to combine some zones, but I will later split the
zones by region, and then merge zones that are very uncommon in a
specific region.
...
*homogenous ecological areas*.

An option is to export one map with just the raw un-scaled slitasje
variable at the polygon level, and another, scaled indicator map
aggregated to either

1.  each *dekningskart* (map of surveyed area)

2.  each vegetation zone (e.g. boreonemal zone in Østlandet; excluding
    zones with too little data points)

3.  each vegetation region (aka bioregion, e.g. boreonemal zone and
    continental section in Østlandet)

4.  each region cross tabulated agains infrastructure index classes

5.  each pixel of a give size, say 1 x 1 km

Option 5 is problematic because we often will have too few points to be
able to estimate credible mean. The second and third option can be good
for certin biotic variables that depends on local species pools ect.
However, slitasje is perhaps best explained by human presence, and
option 4 therefore sounds reasonable.
