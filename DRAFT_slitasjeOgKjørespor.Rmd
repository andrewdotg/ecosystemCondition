...
6.  [Scale](#scale-slitasje-ind) the `slitasje` variable based on reference values.
...
10.  Sample 10 00 random points from an ecosystem delineation and extract indicator values and sd at those points.


11. Export indicator maps and regional extrapolated maps

## About the underlying data
The indicator uses a data set from a standardised field survey of nature types. You can read more about it [here](#naturtyper). I also import a data set called [ANO](#ANO-import)

### Representativity in time and space
The nature type mapping is not random and cannot be said to be area representative. 
The data is from 2018 to present. The data from one field season usually becomes available early the following year. 


### Original units
The variables are recorded on a coarse four-sped ordinal scale (Fig. \@ref(fig:four-step)).

### Temporal coverage
The data goes back to 2018, but is because of data shortage, it is bulked into one time step. I then use the mean date for the raw data, and define the variable as belonging to the year 2020 (read more [here](#scaled-slitasje-variable)). 

### Aditional comments about the dataset
For a run through of the data set, see [here](#naturtyper)

## Ecosystem characteristic
### Norwegain standard
Primærproduksjon

The indicator is tagged to the _Økologisk egenskap_ called Primærproduksjon (Primary productivity). This is tentative, and perhaps _abiotiske forhold_ is more suited. But the thought behind the chiose is that _sliatsje_ affect the potential for primary productivity.

### UN standard
The indicator is tagged as a Physical state characteristics indicator. This is quite clear. It's a tpe of abiotic characteritic that has to do with the physical structure rather than the chemical composition. 
...
```{r}
naturetypes <- readRDS(paste0(pData, "rds/naturtyper.rds"))
```
...
```{r, fig.cap="7SE scores in the naturetype dataset", fig.width=3}
...
```{r, fig.cap="7TK scores in the nature type dataset", fig.width=3}
temp <- naturetypes_wide %>%
  group_by(var_7TK) %>%
  summarise(sum = n())
...
```{r, fig.cap="Counting the number of cases where 7TK or 7SE ws defining of the slitasje indicator score.", fig.width=3}
...
The new data set contains **only polygons**.
...
The GIS layers are used to crop out marine areas, and to define accounting areas, respetively. 
...
2.  take the simplified infrastructure index and
    extract the indicator value that intersect with a given cell, and
    extrapolate the area weighted mean of those values to all similar cell within a
    given region (`ea_spread()`). Errors are calculated from bootstrapping.
    
3. Take the new area weighted indicator values for the homogeneous impact area (HIA) and match them with ecosystem occurrences of the same HIA. The errors are the same as for the HIAs.

3.  calculate an indicator value for a region (accounting area) by doing a weighted
    average based of the relative area of ecosystem occurrences. Errors shold be carried somehow, perhaps via weighted resampling
...
```{r continuous-scale, fig.cap="Showing the conversion from ordinal to (pseudo)continous cale for the slitasje variable", fig.width=5}
...
```{r slitasje-normalise, fig.cap="Performing a non-linear break-point type normalisation of the slitasje variable.", fig.width=5}
...
There is some, but very little variation both between the ecosystems and the four classes in the modified infrastructure index. Note however, that for wetland and semi-natural areas we did not extrapolate values for the most urban areas, and that for naturally open areas we did not extrapolate to the least affected areas.

_These maps are easy to misunderstand, and they should therefor be communicated with caution._

Now, there are multiple way to go proceed. We have indicator maps (vector data), and we have HIA area, and we have the HIA with indicator data (also vector data). 
...
Step 1 above requires an st_interactions on two very large vactor data sets. This should be avoided.


Option 2: 

1. rasterize the extrapolated indicator map, using the ET map as a tamplate, and 
...
2. extract indicator values at those points (from the extrapolated indicator map (a stars object)).

This is perhaps even faster. I will try the last option, even though it will not provide spatailly explicit indicator values for the entire ecosystem delineation, like option 2 does. Perhaps I will revisit option 2 later.
...
```{r}
wetland_points_extracted <- st_extract(wetlands_slitasje_extr, wetland_points)
```
...
```{r, eval=F}
wetlands_slitasje_extr_cropped_reg <- ea_spread(indicator_data = wetlands_slitasje_extr_cropped,
                                      indicator = i_2020,
                                      regions = regions,
                                      groups = ?)
```
...
The calculations of indicator uncertainty is done with bootstrapping, as is described in eaTools.
