...
```{r}
ano <- ano[!is.na(ano$`7SE`),]
```

```{r}
ano$'7TK' <- as.numeric(sub(pattern = "7TK_",
                 replacement = "",
                 x = ano$bv_7tk))
```
...
```{r}
par(mfrow=c(1,2))
barplot(table(ano$`7TK`), xlab="7TK scores")
barplot(table(ano$`7SE`), xlab="7SE scores")
```
