...
##### Combine Naturtype data and ANO
...
assign all the indicator value to the average year of the
data.
...
I want to use the [Homogeneous Impact Areas] (HIA) to define smaller regions into which I can extrapolate the indicator values.
This data is generated by discretizing the Norwegian Infrastructure Index. I refer to the ordinal values of the
four impact area classes as their *Human Impact Factor* (HIF).
...
I also want to split these four HIA classes based on their region. To do this I need the two layers to have the same crs
```{r}
st_crs(HIA) == st_crs(regions)
```

```{r}
HIA <- st_transform(HIA, 25833)
```
...
I now have 20 unique areas that I will, given there is data, extrapolate indicator values over: 
```{r}
HIA_regions <- HIA_regions %>%
  mutate(region_HIF = paste(region, infrastructureIndex))
length(unique(HIA_regions$region_HIF))
```

Let's calculate the areas of these polygons and compare the HIF in the five regions. 
First we need to remove some marine areas.
```{r}
HIA_regions2 <- HIA_regions %>%
...
The figure above shows that Nord-Norge for example, has a lot of relatively untouched areas, and that Østlandet has the highest proportion of impacted areas.
This is expected. 




### Aggregate and spread (extrapolate)
I now want to find the mean indicator value for each HIA (i.e. to aggregate) and to spread these out
spatialy to the entire HIA within each region (i.e. to extrapolate).

```{r, eval=F}
slitasje_data_extrapolated <- ea_spread(indicator_data = slitasje_data,
                         indicator = i_2020,
                         regions = HIA_regions,
                         groups = region_HIF,
                         threshold = 20)
#saveRDS(slitasje_data_extrapolated, "P:/41201785_okologisk_tilstand_2022_2023/data/cache/slitasje_data_extrapolated.rds")
```
...
myBB <- st_bbox(c(xmin=260520.12, xmax = 278587.56,
                ...
                crs = st_crs(slitasje_data_extrapolated))
```

Cropping the raster to the bbox

```{r}
slitasje_data_extrapolated_trd <- sf::st_crop(slitasje_data_extrapolated, myBB)
```
...
```{r}
tm_shape(slitasje_data_extrapolated_trd)+
...
Let's split the ID column to get the HIF
```{r}
slitasje_data_extrapolated <- slitasje_data_extrapolated %>%
  tidyr::separate(ID, into = c("region", "HIF"),
                  sep = " ")
slitasje_data_extrapolated$HIF <- as.numeric(slitasje_data_extrapolated$HIF)
```

Now let's check the correlation between the HIF and the indicator values.
<!-- This is not correct. This count the polygins that have very different sizes -->

```{r slitasje-box, fig.cap="The relationship between human impact factor (a categorical version of the Norwegian Infrastructure Index) and the indicator avleus for slitasje."}
ggplot(slitasje_data_extrapolated, aes(x = factor(HIF), y = meanIndicatorValue))+
  geom_boxplot(fill = "grey",
               size=1.5)+
  theme_bw(base_size = 12)+
  labs(x = "Human Impact Factor", y = "Slitasje indicator value")
```

```{r}
cor(slitasje_data_extrapolated$HIF, slitasje_data_extrapolated$meanIndicatorValue)
```

This relationship is quite good, and I think it justified using HIF as homogeneous areas to extrapolate indicator values onto. 

Split one more time, into regions:
```{r, eval=F}
slitasje_data_extrapolated2 <- ea_spread(indicator_data = slitasje_data_extrapolated,
                         indicator = meanIndicatorValue,
                         regions = regions,
                         groups = region,
                         threshold = 20)
saveRDS(slitasje_data_extrapolated2, "P:/41201785_okologisk_tilstand_2022_2023/data/cache/slitasje_data_extrapolated2.rds")
```

```{r, echo=F}
# silently read cached file
slitasje_data_extrapolated2 <- readRDS("P:/41201785_okologisk_tilstand_2022_2023/data/cache/slitasje_data_extrapolated2.rds")
```

```{r}
plot(factor(slitasje_data_extrapolated2$ID), slitasje_data_extrapolated2$meanIndicatorValue,
     ylab="indicator value", xlab="", ylim=c(0,1))
```
...
But I also need to export a map of the indicator where we extrapolate the indicator values
to the homogeneous areas. This map should be interpreted with caution, since they seem
to provide explicit indicator values for areas where there is no data. This map is mainly for
aggregation purposes. I will mark this map with the prefix _homogeneous-areas_.

I will also export a map of the *homogeneous areas*, over which I will aggregate the indicator values.
This map might be useful in later indicator work.
