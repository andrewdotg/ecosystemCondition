...
#### Validate
I now have 20 unique areas that I will, given there is data, extrapolate
...
corrCheck <- as.data.frame(corrCheck) %>%
  group_by(hovedøkosystem, region) %>%
  summarise(cor = cor(i_2020, infrastructureIndex))
```

```{r, fog.cap="Pearson correaltions between the indicator values at the polygons level and the HIF."}

ggplot(corrCheck, aes(x = hovedøkosystem, 
                      y = cor))+
  geom_bar(stat = "identity")+
  facet_wrap(.~region)+
  coord_flip()
...
From the figures above it appears that the relationship is quite week in general. But the sheer number of data points suggest that all group differences will be very reliable.
...
aggregate) and to spread these out spatialy to the entire HIA within
each region (i.e. to extrapolate).

I will focus on wetland from here on out. Only for open wetlands do we have good exosystem delineation maps to base a spatial averaging on.
...
based on too few data points. I think we need a minimum of 100 data point to base an estimate on.

<!-- NEEDS CACHING -->
```{r}
mySummary <- ea_spread(indicator_data = wetlands,
                         indicator = i_2020,
                         regions = HIA_regions,
                         groups = region_HIF,
                         summarise = TRUE,
...
<!-- Do we need another aggregation step here, to get regional estimates= -->

Let's split the regions column

```{r}
mySummary <- mySummary %>%
  tidyr::separate(region_HIF, into = c("region", "HIF"),
                  sep = " ")
mySummary$HIF <- as.numeric(mySummary$HIF)
```

```{r, fig.cap="Indicator values (area weighted averages) for each region and homogeneous area class, as well as the number of data points per groups."}
ggarrange(
ggplot(mySummary, aes(x = HIF, y = area_weighted_mean_indicator_value,
                      fill = area_weighted_mean_indicator_value))+
  geom_bar(stat = "identity")+
  theme_bw(base_size = 12)+
  facet_wrap(.~region, ncol=1)+
  guides(fill = "none"),
ggplot(mySummary, aes(x = HIF, y = data_points,
                      fill = region))+
  geom_bar(stat = "identity")+
  theme_bw(base_size = 12)+
  facet_wrap(.~region, ncol=1)+
...
The figure above shows that there are much more data on the two middle
classes (class 1 and 2). We do not have enough data to extrapolate to the 
highest impacted areas in any of the regions. These are not the areas prioritised for nature type mapping.

We also see that, even though there are small differences, high HIF is assiciated with lower indicator values.

```{r}
DT::datatable(mySummary)
```
...
```{r, fig.cap="Sliatsje (wetland) indicator extrapolated over Trondheim"}
tm_shape(wetlands_slitasje_extr_trd)+
  tm_polygons(col = "area_weighted_mean_indicator_value",
...
Let's split the ID column to get the HIF and plot agains indicator values

```{r}
wetlands_slitasje_extr <- wetlands_slitasje_extr %>%
  tidyr::separate(ID, into = c("region", "HIF"),
                  sep = " ")
wetlands_slitasje_extr$HIF <- as.numeric(wetlands_slitasje_extr$HIF)
```

```{r}
summary(wetlands_slitasje_extr)
ggplot(wetlands_slitasje_extr, aes(x = factor(HIF), y = area_weighted_mean_indicator_value))+
         geom_boxplot(fill = "grey")+
  theme_bw(base_size = 12)
```

This relationship is quite good, and I think it justified using HIF as
homogeneous areas to extrapolate indicator values onto.


Split one more time, into regions:

```{r, eval=F}
wetlands_slitasje_extr2 <- ea_spread_dev(indicator_data = wetlands_slitasje_extr,
                         indicator = area_weighted_mean_indicator_value,
                         regions = regions,
                         groups = region,
                         summarise = F)
saveRDS(wetlands_slitasje_extr2, "P:/41201785_okologisk_tilstand_2022_2023/data/cache/wetlands_slitasje_extr2.rds")
```

```{r, include=F}
# silently read cached file
wetlands_slitasje_extr2 <- readRDS("P:/41201785_okologisk_tilstand_2022_2023/data/cache/wetlands_slitasje_extr2.rds")
```

```{r}
plot(factor(wetlands_slitasje_extr2$ID), wetlands_slitasje_extr2$area_weighted_mean_indicator_value,
