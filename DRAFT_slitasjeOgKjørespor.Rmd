...
values. I will assign the indicator value to the average year of the
data.
...
```{r, fig.cap="Counting the number of gridd cells for each Human Impact Factor."}
barplot(table(HIA$infrastructureIndex),
        ylab = "Number of 1x1 km grid cells",
        xlab = "Human impact factor")
```
...
This is expected. 
<!-- It also shows that the second HIF class is perhaps to narrowly defined. This is suggested as a change in another PR: -->
...
```{r, echo=F}
# silently read cached file
slitasje_data_extrapolated <- readRDS("P:/41201785_okologisk_tilstand_2022_2023/data/cache/slitasje_data_extrapolated.rds")
```

```{r}
unique(slitasje_data_extrapolated$meanIndicatorValue)
```
<!-- slitasje_data_extrapolated contains 32402 rows, but I expected 5. -->
