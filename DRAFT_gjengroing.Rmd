...
areaCovers <- read_csv('./DATA/From_GEE/area_cover_grid.csv') %>%
...
elevStrata <- st_read('./DATA/From_GEE/elevation_stratified.shp') %>%
...
if (runFromScratch){
  ...
  vaatmarkIndexPolySpat <- st_read('./OUTPUT/vaatmark_index.shp')%>%
    mutate(`system:index` = id,
           elevation = elev, 
           vegClimZoneLab = vegZn) %>%
    dplyr::select(-id, -elev, -vegZn)
  aapneIndexPolySpat <- st_read('./OUTPUT/aapne_index.shp')%>%
    mutate(`system:index` = id,
           elevation = elev, 
           vegClimZoneLab = vegZn) %>%
    dplyr::select(-id, -elev, -vegZn)
...
if (runFromScratch){
  ...
  vaatmarkIndexStrata %>% write_csv('./DATA/vaatmarkIndexStrata.csv')
  aapneIndexStrata %>% write_csv('./DATA/aapneIndexStrata.csv')
  

} else {
  
  vaatmarkIndexStrata <- read_csv('./DATA/vaatmarkIndexStrata.csv')
  aapneIndexStrata <- read_csv('./DATA/aapneIndexStrata.csv')
...
if (runFromScratch){
  ...
  vaatmarkIndexGrid <- st_read('./OUTPUT/vaatmark_index_grid.shp')
  aapneIndexGrid <- st_read('./OUTPUT/aapne_index_grid.shp')
...
if (runFromScratch){
  ...
  vaatmarkIndexRegion <- st_read('./OUTPUT/vaatmark_index_region.shp')
  aapneIndexRegion <- st_read('./OUTPUT/aapne_index_region.shp')
...
```{r}
include_graphics('./DATA/images/val_1.jpg')
```
...
```{r}
include_graphics('./DATA/images/val_2.jpg')
```
...
```{r}
include_graphics('./DATA/images/val_3.jpg')
```
...
```{r}
include_graphics('./DATA/images/val_4.jpg')
```
...
```{r}
include_graphics('./DATA/images/val_5.jpg')
```
...
if (runFromScratch){
  ...
  vaatmarkIndexPolySpat  %>%
    ...
    st_write('./OUTPUT/vaatmark_index.shp', append=FALSE)
  ...
  aapneIndexPolySpat %>% 
    ...
    mutate(id = `system:index`,
           ...
           vegZn = vegClimZoneLab) %>%
    dplyr::select(-"system:index", -vegClimZoneLab, -elevation) %>%
    st_write('./OUTPUT/aapne_index.shp', append=FALSE)
  
  # Grid level
  vaatmarkIndexGrid %>%
    st_write('./OUTPUT/vaatmark_index_grid.shp', append = FALSE)
  aapneIndexGrid %>%
    st_write('./OUTPUT/aapne_index_grid.shp', append = FALSE)
  
  # Region level
  vaatmarkIndexRegion %>%
    st_write('./OUTPUT/vaatmark_index_region.shp', append = FALSE)
  aapneIndexRegion %>%
    st_write('./OUTPUT/aapne_index_region.shp', append = FALSE)
}
