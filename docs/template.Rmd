...
Because this is also a stand-alone Rmd file inside a bookdown project, in order to knit the file for preview purposes,
you need to type rmarkdown::render("template.Rmd") in the console, 
as the the usual Ctrl + Alt + k will not work. When the analyses is complete,
...
and comment on every step in the analysis. Se When completed, Anders will make a copy
of the working file and put it in the same project root folder, but removing the 'DRAFT_' part of the name. 
After modifying it slightly (mainly removing the top YAML header and updating the rmd_files argument in '_bookdown.yml') the output will become available 
on the website (https://ninanor.github.io/ecosystemCondition/). -->

<!-- Examples: For an example, see 'indicators/breareal-workingFile.Rmd'. Notice the degree of annotation.
...
To complete the exmaple, se 'breareal.Rmd' and notice how Anders has prepared the working file to become a
chapter in eth bookdown project. Anders will do this for you as patr of the code review.
-->
...
#### Regions
<!-- In case you need to map the indicator value to regions, you can do that here -->
