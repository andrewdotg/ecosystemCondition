...
```{r withMasking, fig.cap="Compareing computation time before and after masking of raster data. There is no performance increase by masking before aggregating."}
autoplot(
  microbenchmark(
No_masking =  x_aug %>%
  st_apply(1:2, mean) %>%
...
start_month <-  "06"
end_month <- "08"
myFiles <- list.files(path2, full.names = T)
