...
I will remove the _band_ dimension.
```{r}
...
I need to code a for loop for importing the .nc files. 

For then selecting the attribute, simply do like this:
...
```{r, fig.cap="Demonstrating the effecty of maskig the dummy data using a perfectly aligne raster ET map."}
x_masked <- x_aug
x_masked[is.na(dummy_et)] <- NA
plot(x_masked[,,,1])
```

```{r, fig.cap="There is no performance increas by making before aggregating"}
autoplot(
  microbenchmark(
No_masking =  x_aug %>%
  st_apply(1:2, mean) %>%
...
If the reference value was static, or if there was a pattern in v_1970, the scaled indicator map would look different.
...
Calculate diference between indicator and reference
We don't want to include this in ts, but as a temp file.
```{r}
tsX <- ts1
```
