...
But to create some differences between the variable and the reference values, I will tweak these slightly, more or less divide them by two (pluss some error)

```{r}
y1v <- y1 %>% mutate(Attribute_A = Attribute_A*rnorm(1,.5,.3)) %>% filter(band == 1) %>% adrop() 
y2v <- y2 %>% mutate(Attribute_A = Attribute_A*rnorm(1,.5,.3)) %>% filter(band == 1) %>% adrop() 
y3v <- y3 %>% mutate(Attribute_A = Attribute_A*rnorm(1,.5,.3)) %>% filter(band == 1) %>% adrop() 
y4v <- y4 %>% mutate(Attribute_A = Attribute_A*rnorm(1,.5,.3)) %>% filter(band == 1) %>% adrop() 
y5v <- y5 %>% mutate(Attribute_A = Attribute_A*rnorm(1,.5,.3)) %>% filter(band == 1) %>% adrop() 
...
```{r}
system.time(
  temp <- st_apply(y_v_merged, c("x", "y"), FUN = function(Attribute_A,reference_value) Attribute_A/reference_value)
)
```
