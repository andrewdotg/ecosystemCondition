...
path2 <- paste0(path, "GeoSpatialData/Meteorology/Norway_SeNorge2018_v22.09/Original/rr_tm/") #seNorge2018_1957.nc

myFiles <- list.files(path2, full.names = T)[1:2]
start_month <-  "06"
end_month <- "08"
...
for(i in 1:length(myFiles)){
  ...
  tic("init")
  temp <- stars::read_ncdf(paste(myFiles[i]), var="tg")
  year_temp <- year(st_get_dimension_values(temp, "time")[1])
  start <- ym(paste(year_temp, start_month, sep="-"))
  end <-  ym(paste(year_temp, end_month, sep="-"))
  myInterval <- interval(start, end)
