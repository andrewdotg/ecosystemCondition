...
But to create some differences between the variable and the reference values, I will tweak these slightly, more or less divide them by two. I then merge with the reference value and sd, creating one common data cube. 

```{r}
y_var <- y %>% mutate(Attribute_A = Attribute_A/2) %>% 
  split(3) %>% c(y_ref)
y_var <- y_var %>%
...
# Mutate 
y_var_norm <- y_var %>%
  mutate(across(all_of(cols), ~.x / reference)) %>%
