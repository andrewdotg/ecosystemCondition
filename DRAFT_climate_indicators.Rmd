...
For this example I want to calculate the mean temperature over the summer. I therefore need to aggregate over time. Here is how.
...
Actually, this second method is better because we don't need to filter out additional dimensions, and it retains the attribute name (the first method replaced it with the function name *mean*).
...
y1 <- x_summerMean_v2
y2 <- x_summerMean_v2 %>% st_set_dimensions("time", values = oldTime %m+% years(-1)) %>% mutate(Attribute_A = Attribute_A*rnorm(1,1,.3)) 
y3 <- x_summerMean_v2 %>% st_set_dimensions("time", values = oldTime %m+% years(-2)) %>% mutate(Attribute_A = Attribute_A*rnorm(1,1,.3))
y4 <- x_summerMean_v2 %>% st_set_dimensions("time", values = oldTime %m+% years(-3)) %>% mutate(Attribute_A = Attribute_A*rnorm(1,1,.3))
y5 <- x_summerMean_v2 %>% st_set_dimensions("time", values = oldTime %m+% years(-4)) %>% mutate(Attribute_A = Attribute_A*rnorm(1,1,.3))
...
```{r, eval=F, include=F}
# I would normally do the above code chink using tmap, but I cant get it to work when faceting across dimensions:
tm_shape(y)+
  tm_raster()+
  tm_facets("time")
```
...
It's easier if median and sd are unique attributes, rather than levels of a dimension.
...
But to create some differences between the variable and the reference values, I will tweak these slightly, more or less divide them by two (plus some error). Actually, just leaving the reference layer and the variable layer perfectly correlated causes an error when dividing the dimension using st_apply.

```{r}

myRandom <- function(x) x*rnorm(1,1,.1)/2
myRandom(1)

y1v <- y1 %>% filter(band == 1) %>% adrop() %>% st_apply(1:2, FUN = myRandom)
y2v <- y2 %>% filter(band == 1) %>% adrop() %>% st_apply(1:2, FUN = myRandom) 
y3v <- y3 %>% filter(band == 1) %>% adrop() %>% st_apply(1:2, FUN = myRandom) 
y4v <- y4 %>% filter(band == 1) %>% adrop() %>% st_apply(1:2, FUN = myRandom) 
y5v <- y5 %>% filter(band == 1) %>% adrop() %>% st_apply(1:2, FUN = myRandom) 


#y_v <- c(y1v, y2v, y3v, y4v, y5v, y_ref)

y_v <- c(y1v, select(y_ref, reference_value))
y_v_merged <- merge(y_v)
dim(y_v_merged)
names(y_v_merged)
y_v_merged <- setNames(y_v_merged, "value")

st_get_dimension_values(y_v_merged, "attributes")
```
...
#### Normalise variable 
I now want to to cell-by-cell computations across the _attributes_ dimension, and divide _Attribute_A_ by _reference_value_.
