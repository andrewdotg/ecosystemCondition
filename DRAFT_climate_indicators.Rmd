...
myFiles <- list.files(path2, pattern=".nc$",full.names = T)[1:2]
start_month <-  "06"
end_month <- "08"
...
julian_start <- yday(st_get_dimension_values(temp, "time")[1] %m+% months(+start_month_num))
julian_end <- yday(st_get_dimension_values(temp, "time")[1] %m+% months(+end_month_num))
...
tif = system.file("tif/L7_ETMs.tif", package = "stars")
t1 = as.Date("1970-05-31")
restProxy = read_stars(tif,
               proxy = T)
class(restProxy)
restProxy2 <- st_as_stars(restProxy)
class(restProxy2)
dim(restProxy)

restProxy3 <- restProxy %>% filter(band==1)
dim(restProxy3)
restProxy4 <- st_as_stars(restProxy3)
dim(restProxy4)

restProxy5 <- restProxy %>% filter(band==1) %>%
  st_apply(1:2, mean)
dim(restProxy5)
restProxy6 <- st_as_stars(restProxy5)
dim(restProxy6)

restProxy7 <- restProxy %>% filter(band==1) 
restProxy7 <- restProxy7 %>%  st_apply(1:2, mean)
dim(restProxy7)
restProxy8 <- st_as_stars(restProxy7)
dim(restProxy8)
...
```{r}
dim(temp)
st_get_dimension_values(temp, "time")
#
#tic("execute")
#
real_temp_summer2 <- st_as_stars(real_temp_summer)
real_temp_summer <- st_as_stars(real_temp_summer)

#toc()
temp <- st_as_stars(temp)


write_stars(temp, "P:/41201785_okologisk_tilstand_2022_2023/data/climate_indicators/dummy_data/temp_test.tif")
...
st_get_dimension_values(real_temp_summer, "time")
```
