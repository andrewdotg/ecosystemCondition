...
```{r}
p = st_sfc(st_point(c(7039947.63, 270783.69)), crs=25833)
```
...
```
Remoce marine areas from regions
...
```{r, fig.cap="Five accounting areas (regions)."}
...
# The last file, the last year, is incomplete and dont include all juliani dates that we select below
...
for(i in 1:length(myFiles)){
  ...
  lookup <- setNames("mean", paste0("v_", year_temp)) # Perhaps leave out the v_ to get a numeric vector instead, which is easier to subset
  st_crs(temp) <- 25833
  toc()
...
This takes about 20 sec per file/year, or 22 min on total. Here's a test for the effect of splitting over more cores.
```{r, eval=F}
cl <- makeCluster(2L)
cl2 <- makeCluster(6L)
cl3 <- makeCluster(10L)
...
```{r, eval=F}
saveRDS(real_temp_summer, "/data/P-Prosjekter2/41201785_okologisk_tilstand_2022_2023/data/climate_indicators/aggregated_climate_time_series/temp_median_summer.rds")
write_stars(real_temp_summer, "/data/P-Prosjekter2/41201785_okologisk_tilstand_2022_2023/data/climate_indicators/aggregated_climate_time_series/temp_median_summer.tiff")
```

These files are about 500 MB, but the tiff file reads in a lot quicker then the rds file, so from now on I'll just export as tiff.

```{r}
temp_median_summer <- read_stars("/data/P-Prosjekter2/41201785_okologisk_tilstand_2022_2023/data/climate_indicators/aggregated_climate_time_series/temp_median_summer.tiff",
                                 proxy=F)
# This is too slow:
# readRDS("/data/P-Prosjekter2/41201785_okologisk_tilstand_2022_2023/data/climate_indicators/dummy_data/temp_median_summer.rds")
# but it could be that RData works better 
```

```{r, eval=T}
names(real_temp_summer)
```

```{r, eval=T}
dim(real_temp_summer)
```

```{r, eval=T}
st_get_dimension_values(real_temp_summer, "Year")
```



Note that GTiff automatically renames the third dimension _band_ and also renames the attribute.

```{r}
st_get_dimension_values(temp_median_summer, "band")
```

I can rename them. 
```{r}
temp_median_summer <- temp_median_summer %>% 
  st_set_dimensions(names = c("x", "y", "v_YEAR")) %>%
  setNames("temperature")
dim(temp_median_summer)
```

```{r, fig.cap="Showing three random slices of the year dimension."}
ggplot()+
  geom_stars(data = temp_median_summer[,,,c(1,11,22)], downsample = c(10, 10, 0)) +
  coord_equal() +
  theme_void() +
  scale_fill_viridis_c(option = "D") +  
  scale_x_discrete(expand = c(0, 0)) +
...
We need to first to define a reference period and then to subset our data `temp_median_summer`. We can practice on the dummy data `y`.
...
```{r}
new_dims <- as.Date(paste0(substr(st_get_dimension_values(temp_median_summer, "v_YEAR"), 3, 6), "-01-01"))
temp_median_summer_ref <- temp_median_summer %>%
...
```{r}
temp_median_summer_ref <- temp_median_summer_ref %>%
  filter(v_YEAR %within% interval("1961-01-01", "1990-12-31"))
st_get_dimension_values(temp_median_summer_ref, "v_YEAR")
```
...
#write_stars(temp_median_summer_ref, "/data/P-Prosjekter2/41201785_okologisk_tilstand_2022_2023/data/climate_indicators/aggregated_climate_time_series/temp_median_summer_ref.tiff")

saveRDS(temp_median_summer_ref, "/data/P-Prosjekter2/41201785_okologisk_tilstand_2022_2023/data/climate_indicators/aggregated_climate_time_series/temp_median_summer_ref.RData")
```

```{r, eval=T, include=F}
temp_median_summer_ref <- readRDS("/data/P-Prosjekter2/41201785_okologisk_tilstand_2022_2023/data/climate_indicators/aggregated_climate_time_series/temp_median_summer_ref.RData")
```

```{r, eval=T}
temp_median_summer_ref_long <- temp_median_summer_ref %>% split("median_sd")
```

The attribute name is *mean*. We can change this to something more meaningful.

```{r, eval=T}
temp_median_summer_ref_long <- setNames(temp_median_summer_ref_long, c("reference_upper", "sd"))
```

```{r, fig.cap="Showing the upper reference levels and the standard deviation from actual data of median summer temperatures."}
tmap_arrange(
tm_shape(st_downsample(temp_median_summer_ref_long, 10))+
  tm_raster("reference_upper")
,
tm_shape(st_downsample(temp_median_summer_ref_long, 10))+
  tm_raster("sd",
...
```{r}
y_var <- temp_median_summer %>%
  split(3) %>%
  c(temp_median_summer_ref_long)
...
```{r, eval=F}
gc()
saveRDS(y_var_norm, "/data/P-Prosjekter2/41201785_okologisk_tilstand_2022_2023/data/climate_indicators/aggregated_climate_time_series/temp_median_summer_normalised.RData")
```
...
The _real_ indicator values should be running means over 5 years, but calculating a running mean for all time steps is too time consuming. Therefore, the time series will be presented with yearly resolution, and the scaled, regional indicator values will be calculated for distinct time steps.


#### Plot time series as line graphs

We could also plot a time series as a series of maps, but then we need to avergae over this 5 time periods firs. We can get to that later.
...
However, this does not preserve the region name, and a subsequent st_intersection taks a little whie and gives some boundary problems.

I can also converting att pixels to points and then do the intersection. 
...
I can rastereze the regions data set, turn it into a coded dimension and use st_apply, but thats tedious. 

Let me try using `exactextract`, as a parallel to `aggregate`. This requires us to turn to the `terra` package for a bit. 
...
```{r, eval=F}
(
regional_means_sd <- rast(y_var_norm) %>%
  exact_extract(reg, fun = function(X) c(mean = mean(x), sd = sd(x)), append_cols = "region", progress=T) %>%
  setNames(c("region", names(y_var_norm)))
  )
```
...
```{r}
(
i_2022 <- regional_means_long %>%
  filter(year >2017) %>%
  group_by(region) %>%
  summarise(i_2022 = mean(i))
)
```

```{r}
(
sd_2022 <- regional_means_long %>%
  filter(year >2017) %>%
  group_by(region) %>%
  summarise(i_2022 = mean(i))  
)
