...
Firt, let me cut the _band_ dimension.
```{r}
...
myRandom <- function(x) x*rnorm(1,1,.1)
...
But to create some differences between the variable and the reference values, I will tweak these slightly, more or less divide them by two.
...
y1v <- y1 %>% adrop() %>% mutate(Attribute_A = Attribute_A/2)
