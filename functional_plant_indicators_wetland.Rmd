...
```{r, include = T, attr.output='style="max-height: 300px;"'}
...
```{r}
head(ANO.sp.ind) %>%
  kable("html") %>% kable_styling("striped") %>% scroll_box(width = "100%")
```
...
```{r fpiw-plot-maintype,echo=FALSE,message=FALSE, warning=FALSE, fig.cap="Scaled vegetation indicator values for wetlands. 1 = cases where variable is lower than the reference value- 2 = cases where variablei si higher than the reference value."}
...
```{r ph-lower-map-fpiw, echo=FALSE, warning=FALSE,message = FALSE, fig.cap="Functional plant indicator values for pH (lower) in wetlands."}
#### scaled value maps ####
