...
unique(SentinelNDVI.seminat.train$tilstand)
SentinelNDVI.seminat.train <- SentinelNDVI.seminat.train %>% mutate(tilstand_num = recode(tilstand, 
                                            "God" = '0',
                                            "Moderat" = '1',
                                            "Redusert" = '2',
                                            "Svært redusert" = '3'))
SentinelNDVI.seminat.train$tilstand_num <- as.numeric(SentinelNDVI.seminat.train$tilstand_num)

# check if there's any 0s or 1s (which beta cannot handle)
summary(SentinelNDVI.seminat.train$mean_beta)
...
T33/T44: NDVI increases as condition deteriorates
...
# check if there's any 0s or 1s (which beta cannot handle)
summary(SentinelNDVI.seminat$mean_beta)
summary(ModisNDVI.seminat$mean_beta)
summary(LandsatNDVI.seminat$mean_beta)
# replace 1s in Landsat data with 0.9999
LandsatNDVI.seminat <- LandsatNDVI.seminat %>%
  mutate(mean_beta = replace(mean_beta, mean_beta == 1, 0.9999))
...
```{r message=FALSE, warning=FALSE}
model.seminat.time.tilst.V9V10.Sent <- glmmTMB(mean_beta~year*tilstand + (1|id), family=beta_family(),data=SentinelNDVI.seminat[SentinelNDVI.seminat$hovedtype %in% c("V9","V10"),])
model.seminat.time.tilst.V9V10.Modi <- glmmTMB(mean_beta~year*tilstand + (1|id), family=beta_family(),data=ModisNDVI.seminat[ModisNDVI.seminat$hovedtype %in% c("V9","V10"),])
model.seminat.time.tilst.V9V10.Land <- glmmTMB(mean_beta~year*tilstand + (1|id), family=beta_family(),data=LandsatNDVI.seminat[LandsatNDVI.seminat$hovedtype %in% c("V9","V10"),])
summary(model.seminat.time.tilst.V9V10.Sent)$coefficients$cond
summary(model.seminat.time.tilst.V9V10.Modi)$coefficients$cond
summary(model.seminat.time.tilst.V9V10.Land)$coefficients$cond
```
