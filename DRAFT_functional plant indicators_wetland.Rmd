...
- as testing data we use plant community data from the ANO monitoring scheme (cf. Tingstad et al. 2019)
...
ANO stands for 'areal-representativ naturovervåking', i.e. 'area representative nature monitoring'. Within a 5-year cycle, 1000 sites are  randomly distributed across mainland Norway. Each ANO site spans a 500 x 500 m grid cell and the data collection at each ANO site includes up to 12 evenly distributed vegetation analyses in 1 x 1 m plots (up to 12, because some of these evenly distributed points may be in water or otherwise inaccessible). For the vegetation analyses, the cover of each vascular plant species in the plot is recorded. Every vegetation analysis is accompanied by an assessment of the ecosystem the plot lies in, including ecosystem type and some additional variables related to ecosystem-specific drivers of state. In the wetland-analysis in this document, we only use the plots which were classified as on of the wetland ecosystem types in the Norwegian categorization system for eco-diversity (NiN).
...
For wetlands, the ANO data contain 1351 plots in 330 sites, in principle distributed randomly across the country. As wetlands occur more often in certain regions of Norway than in others the amount of plots and sites is not equal among the five regions ecological condition is assessed for (Northern Norway, Central Norway, Eastern Norway, Western Norway, Southern Norway). The in total 1351 plots are distributed across regions in the following way:
