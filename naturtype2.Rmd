...
num <- aggregate(data = dat2,
              km2~naturtype,
              FUN = sum)

num$numberOfLocalities <- aggregate(data = dat2,
              km2~naturtype,
              FUN = length)[,2]
#num <- num[num$naturtype %in% ntypDF2$Nature_type,]
...
```{r, fig.cap="The total number of mapped localities for nature types associated with three selected main ecosystems"}
ntyp_fill %>%
  arrange(numberOfLocalities) %>%
  mutate(Nature_type=factor(Nature_type, levels=Nature_type)) %>%   # This trick update the factor levels
  ggplot( aes(x = Nature_type,
               y = numberOfLocalities)) +
    geom_segment( aes(xend=Nature_type, yend=0)) +
