...
### Data sets
The workflow relies on two major sources of data: the up-to-date raw data from "hekkefuglovervåkingen" and the calculated abundance indices from the predecessor monitoring schemes. Additionally, we also a couple of lists holding information on which species (and years) are to be included in which parts of the analytical workflow.  

#### Monitoring data from "Hekkefuglovervåkingen"
The route-based raw data from "hekkefuglovervåkingen" are compiled after each breeding season and stored in NINA's database. From that database we download the data relevant for the PECBMS Trim analyses and for the specified range of years (here from 2006 up to and including 2022). 
...
```{r, echo = F}
print(Spp_selection$Species[which(Spp_selection$EURINGCode %in% sppLists$sppLists$MSI_farmland)])
``` 
Forest MSI species:
```{r, echo = F}
print(Spp_selection$Species[which(Spp_selection$EURINGCode %in% sppLists$sppLists$MSI_forest)])
``` 

Mountain MSI species:
```{r, echo = F}
print(Spp_selection$Species[which(Spp_selection$EURINGCode %in% sppLists$sppLists$MSI_mountain)])
``` 
Wetlands MSI species:
```{r, echo = F}
print(Spp_selection$Species[which(Spp_selection$EURINGCode %in% sppLists$sppLists$MSI_wetlands)])
``` 
...
```{r, echo = FALSE}
MSI_results <- readRDS("data/MSI_results.rds")
MSI_longTerm <- MSI_results$MSI_baseline1996
MSI_all <- MSI_results$MSI_baseline2008
```
...
Now we'll calculate indices for all ecosystems using the same setup, but using 2008 as a reference year for all of them to make values comparable: 
