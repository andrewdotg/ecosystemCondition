# Functional plant indicators, naturally open ecosystems below the tree line (Grime's CSR values, Light, Nitrogen, Soil disturbance) {#Functional-plant-indicators-seminat}
...
In the 'functional plant indicator' project for nat-open ecosystems, we use four sets of data for building indicators for ecological condition:

- as test data we use plant community data (1) from the ANO monitoring scheme (cf. Tingstad et al. 2019 for the years 2019-2021, which among many other ecosystem types contains the nat-open types "Åpen grunnlendt mark" (T2), coastal meadows (T12), "meadows and heath in rock slides"Rasmarkhei og -eng" (T16), and "we also decided to include near-semi-natural meadows"Åpen flomfastmark" (T18) ) and (2) from the GRUK monitoring scheme (cf. Evju et al. 2020), which focuses on limestone-rich "Åpen grunnlendt mark" (T2) around the Oslofjord.
