...
# NDVI varies between hovedtyper: highest in seminat wet-meadows (V10), lowest in ombrotrophic bogs (V3)
...
# NDVI varies between ecosystem subtypes in V1. For the other types things seem more constant, but there's also less data and less minor ecosystem types present in the data. 
...
SentinelNDVI.wetland %>%
  ...
  geom_point(size=0.7, shape=16, color="grey") +
  facet_wrap( ~hovedtype)
# in seminat types (V9,V10), kaldkilde (V4), and bogs (V3) ndvi seems to increase as condition deteriorates, in fens (V1) not much of a pattern
...
In semi-natural wetland ecosystems (V9, V10) NDVI increases as ecosystem condition deteriorates, and this is consistent for the entire country.
In natural fens (V1) NDVI increases in the South but decreases in the North as ecosystem condition deteriorates. In bogs (V3) there is no consistent pattern.
