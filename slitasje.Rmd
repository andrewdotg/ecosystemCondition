...
```{r slitasje-naturetype, fig.cap="ADSV scores.", fig.width=3}
temp <- naturetypes_wide %>%
  group_by(slitasje) %>%
  summarise(sum = n())
...
It appears most localities are in good condition. Note also that we can get >100% for the unscaled variable.  
...
First I extract the mid values for each ordinal range step, like I did previously for the nature type data set.
...
ggplot(temp, aes(x = as.numeric(Var1),
                 y = Freq))+
  geom_bar(stat="identity",
           fill="grey",
           colour = "black")+
