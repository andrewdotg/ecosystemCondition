...
This is in the wrong CRS. 

```{r}
mireET <- sf::st_transform(mireET, 25833)
```
...
```{r, eval=F}
write_stars(mireET_warped, paste0(pData, "wetland_ecosystem_EPSG25833_50m.tif"))
```



```{r}
wetlands_trd <- sf::st_crop(mireET, myBB)
tm_shape(wetlands_trd)+
  tm_raster()+
  tm_shape(hw_utm)+
  tm_lines(col="blue")
```
