...
-   indicator maps (vector data, small polygons), and we have

-   total area for each HIA class and region combo, and we have the

-   extrapolated indicator maps (HIA maps with aggregated indicator
    values and errors, also vector data, wall-to-wall with some NA areas).
...
So I will transform the extrapolated indicator map ti match the ET instead. Note that the ET map is just for southern Norway up to about the middle of the country.

```{r}
system.time(
  wetlands_slitasje_extr <- st_transform(wetlands_slitasje_extr, st_crs(mireET))
)
```

Then I import the real ET data and mask
...
<!-- PLAN: rasterize wetland_slitasje_extr and c() with mireET, and mask -->
<!-- Or use  -->
<!-- wetlands_slitasje_extr[mireET==0] <- NA -->

```{r}
wetlands_slitasje_extr_stars <- stars::st_as_stars(wetlands_slitasje_extr)
temp <- c(wetlands_slitasje_extr_masked, mireET)
class(wetlands_slitasje_extr)
dim(temp)
```
