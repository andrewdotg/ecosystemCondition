...
Sentinel-2 satellite NDVI data span the years 2015-2022.
...
```
And we upload maps for Norway and the five regions: Southern, Western, Eastern, Central, adn Northern Norway
```{r}
...
### Regression analyses, NDVI as a function of condition, Sentinel-2
```{r}
...
summary( betareg(mean_beta~tilstand_num*region*hovedtype, data=SentinelNDVI.wetland.train) )
# full model does not work, remove the 3-way interaction
summary( betareg(mean_beta~tilstand_num + region + hovedtype + tilstand_num:region + tilstand_num:hovedtype + region:hovedtype, data=SentinelNDVI.wetland.train) )
# this works! Let's run a stepwise-function to fin the most parsimonious model
model.wetland.train <- betareg(mean_beta~tilstand_num + region + hovedtype + tilstand_num:region + tilstand_num:hovedtype + region:hovedtype, data=SentinelNDVI.wetland.train)
model.wetland.train <- StepBeta(model.wetland.train)
summary(model.wetland.train)
```
...
Since the 3-way interaction term was not possible to be evaluated by the model, but it is evident that all three variables matter, we run the model separately for every region
```{r}
model.wetland.train.N <- betareg(mean_beta~tilstand_num*hovedtype, data=SentinelNDVI.wetland.train[SentinelNDVI.wetland.train$region=="Northern Norway",])
model.wetland.train.C <- betareg(mean_beta~tilstand_num*hovedtype, data=SentinelNDVI.wetland.train[SentinelNDVI.wetland.train$region=="Central Norway",])
model.wetland.train.W <- betareg(mean_beta~tilstand_num*hovedtype, data=SentinelNDVI.wetland.train[SentinelNDVI.wetland.train$region=="Western Norway",])
model.wetland.train.E <- betareg(mean_beta~tilstand_num*hovedtype, data=SentinelNDVI.wetland.train[SentinelNDVI.wetland.train$region=="Eastern Norway",])
model.wetland.train.S <- betareg(mean_beta~tilstand_num*hovedtype, data=SentinelNDVI.wetland.train[SentinelNDVI.wetland.train$region=="Southern Norway",])
